...
def prepare_training_data(grouped_data: Dict[str, List[str]]) -> List[Tuple[str, Dict]]:
    ...
    # Używamy GUEST, HOST i MAYBE (jako GUEST z mniejszą wagą) do treningu
    labels_to_use = ['GUEST', 'HOST', 'MAYBE']
    ...
    for label in labels_to_use:
        ...
        # Mapuj MAYBE na GUEST dla treningu
        target_label = 'GUEST' if label == 'MAYBE' else label
        
        if label == 'MAYBE':
            print(f"🔮 Przetwarzam {len(phrases)} fraz MAYBE jako GUEST (z mniejszą wagą)")
        else:
        ...
        for phrase in phrases:
            ...
            entities = [(start, end, target_label)]
            
            # Dodaj wagę dla przykładów MAYBE (0.5)
            if label == 'MAYBE':
                training_data.append((text, {"entities": entities, "weight": 0.5}))
            else:
    ...
    # Pokaż statystyki
    regular_count = sum(1 for _, annotations in training_data if "weight" not in annotations)
    weighted_count = sum(1 for _, annotations in training_data if "weight" in annotations)
    print(f"   • Standardowe przykłady: {regular_count}")
    print(f"   • Przykłady MAYBE (waga 0.5): {weighted_count}")
...
def create_spacy_examples(nlp, training_data: List[Tuple[str, Dict]]) -> List[Example]:
    ...
    for text, annotations in training_data:
        try:
            # Usuń wagę z annotations przed tworzeniem przykładu
            weight = annotations.pop("weight", 1.0)
            
            doc = nlp.make_doc(text)
            example = Example.from_dict(doc, annotations)
            
            # Jeśli waga < 1.0, dodaj przykład mniej razy (symulacja niższej wagi)
            if weight >= 1.0:
                examples.append(example)
            elif weight >= 0.5:
                # Dla wagi 0.5, dodaj co drugi raz (50% szansy)
                import random
...
def train_and_save_ner_model(feedback_file: str = "data/feedback.json", 
                             model_output_dir: str = "models/ner_model_2025_08_03",
                             n_iter: int = 30) -> bool:
    """
    ...
    training_labels = ['GUEST', 'HOST', 'MAYBE']
    ...
    maybe_count = len(grouped_data.get('MAYBE', []))
    
    print(f"✅ Będę trenować na {available_training_data} przykładach:")
    print(f"   • GUEST: {guest_count}")
    print(f"   • HOST: {host_count}") 
    print(f"   • MAYBE→GUEST (waga 0.5): {maybe_count}")
